...
API_ID_FIELD = 'id'
API_PATH = '/api/'
API_PART_SEND_MAIL = '_mail'
API_SEND_MAIL = '/' + API_PART_SEND_MAIL
API_DATA_PATH = 'db'
...
EMAIL_FROM = 'noresponder@automaticemail.com'

PATH_ROOT = parser.get('path_server', 'Root')
...
class SparrestHandler(SimpleHTTPRequestHandler):
    ...
    def is_sendmail_request(self):
        """Checks if the request is referred to an SENDMAIL item"""
        return self.path[:len(API_SEND_MAIL)] == API_SEND_MAIL
    ...
    def get_posts_resource_parts(self):
        """
        Returns a list of resource parts: if URL is 'API_PATH/foo/bar' it returns ['foo', 'bar']
        If not is a valid API_REQUEST, returns an empty list
        """
        if not (self.is_api_request() or self.is_sendmail_request()):
            return []
    ...
    def send_mail(self):
        ...
        try:
            message = 'Subject: %s\n\n%s' % (data['subject'], data['msg'])
            self.mail_server.connect(EMAIL_HOST, EMAIL_PORT)
            self.mail_server.sendmail(EMAIL_FROM, data['to'], message)

            self.write_send_mail_ok()
        except Exception as e:
            self.write_send_mail_error(e)
    ...
    def write_send_mail_ok(self):
        self.write_response({
            200: 'Send message success'
        }, 200)

    def write_send_mail_error(self, err):
        self.write_response({
            400: 'Sending mail error: {0}'.format(err.message)
        }, 400)
    ...
    def do_POST(self):
        ...
        resource_parts = self.get_posts_resource_parts()
        if resource_parts is None:
            self.write_method_not_allowed_response()
            ...
            self.write_invalid_api_uri_format_response()
        elif resource_parts[0] == API_PART_SEND_MAIL:
            self.send_mail()
        else:
